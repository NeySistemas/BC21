...
public class AsociationServiceImp implements AsociationService {
    ...
    public Mono<Asociation> create(String idCustomer, String idProduct, float amount) {
        ...
        Customer customer=restTemplate.getForObject("http://customer-service/mngcustomers/cus/"+idCustomer, Customer.class);
        CategoryCustomer categoryCustomer=restTemplate.getForObject("http://customer-service/mngcustomers/cat/"+customer.getIdCategory(), CategoryCustomer.class);
        Product product=restTemplate.getForObject("http://product-service/mngproducts/pro/"+idProduct, Product.class);
        CategoryProduct categoryProduct=restTemplate.getForObject("http://product-service/mngproducts/cat/"+product.getIdCategory(), CategoryProduct.class);
